...
def get_stats(model):
    ...
    module_dict, all_layers = get_layers(model)

    for layer in all_layers:

        weight_mask = all_layers[layer].weight
        #bias_mask = all_layers[layer].bias
...
def get_layers(model):
    ...
    for name, module in model.named_modules():
        if not (isinstance(module, resnet.ResNet)|isinstance(module, nn.Sequential)|isinstance(module, resnet.BasicBlock)|isinstance(module, resnet.LambdaLayer)):
...
def compare(stats, models, epochs, title = "Comparison of Accuracy and Loss"):
    """Compares training accuracy and loss, test accuracy and loss of given models,
        was only used during practise runs to ensure plausible behaviour
